module PrimitivesMetrics
  export barotropic_evaluate, spectral_evaluate, CFSR_evaluate, plot_Lat_Lon_mesh, plot_Lat_Lon_CFSR, l2_dist, normalized_errors, time_horizon, barotropic_plot_error, spectral_plot_error, CFSR_plot_error
  ...
  function CFSR_evaluate(model_filepath, save_name)
      #-- To be used on saved outputs from ESNs trained on spectral_dynamics system
      #-- Inputs: filepath of a trained model
      #-- Outputs: Plots of ________

      # Determine paths from name
      save_path = "./eval/results/$save_name/"
      mkdir(save_path)

      # Load the predictions and ground truths
      #-- Shapes are [longitude, latitude, height, time]
      #-- u is eastward velocity, v is northward velocity, P is surface pressure, T is temperature
      # NOTE: only using first 500 hrs of each for now to save memory
      file = jldopen(model_filepath,"r",mmaparrays=true)
      pred_u_grid = read(file,"pred_u_grid")
      pred_u_grid = pred_u_grid[:,:,:,1:500]
      test_u_grid = read(file,"test_u_grid")
      test_u_grid = test_u_grid[:,:,:,1:500]
      println("u loaded. ...")
      pred_v_grid = read(file,"pred_v_grid")
      pred_v_grid = pred_v_grid[:,:,:,1:500]
      test_v_grid = read(file,"test_v_grid")
      test_v_grid = test_v_grid[:,:,:,1:500]
      println("v loaded. ...")
      pred_P_grid = read(file,"pred_P_grid")
      pred_P_grid = pred_P_grid[:,:,:,1:500]
      test_P_grid = read(file,"test_P_grid")
      test_P_grid = test_P_grid[:,:,:,1:500]
      println("P loaded. ...")
      pred_T_grid = read(file,"pred_T_grid")
      pred_T_grid = pred_T_grid[:,:,:,1:500]
      test_T_grid = read(file,"test_T_grid")
      test_T_grid = test_T_grid[:,:,:,1:500]
      println("T loaded. ...")
      ...
      # Rotate arrays, needed for CFSR for proper viewing.
      # WARNING: UNTESTED, CHECK THIS
      test_u_grid = reverse(reverse(test_u_grid,dims=1),dims=2)
      test_v_grid = reverse(reverse(test_v_grid,dims=1),dims=2)
      test_P_grid = reverse(reverse(test_P_grid,dims=1),dims=2)
      test_T_grid = reverse(reverse(test_T_grid,dims=1),dims=2)
      pred_u_grid = reverse(reverse(pred_u_grid,dims=1),dims=2)
      pred_v_grid = reverse(reverse(pred_v_grid,dims=1),dims=2)
      pred_P_grid = reverse(reverse(pred_P_grid,dims=1),dims=2)
      pred_T_grid = reverse(reverse(pred_T_grid,dims=1),dims=2)

      # Plot errors
      # Timestep to end plot for a zoomed in version
      t_end = 50
      CFSR_plot_error(u_errors[1:t_end], v_errors[1:t_end], P_errors[1:t_end], T_errors[1:t_end], save_file_name=save_path*"zoom_error")
      # plot all timesteps
      CFSR_plot_error(u_errors, v_errors, P_errors, T_errors, save_file_name=save_path*"error")

      # Create gifs of evolving system
      plot_Lat_Lon_CFSR(test_u_grid, pred_u_grid, parameter="Eastward Velocity (m/s)", level=1, timestep=1, animation=true, save_file_name=save_path*"u")
      plot_Lat_Lon_CFSR(test_v_grid, pred_v_grid, parameter="Northward Velocity (m/s)", level=1, timestep=1, animation=true, save_file_name=save_path*"v")
      plot_Lat_Lon_CFSR(test_P_grid, pred_P_grid, parameter="Surface Pressure (Pa)", level=1, timestep=1, animation=true, save_file_name=save_path*"P")
      plot_Lat_Lon_CFSR(test_T_grid, pred_T_grid, parameter="Temperature (K)", level=1, timestep=1, animation=true, save_file_name=save_path*"T")
  ...
  function plot_Lat_Lon_CFSR(in_true_grid, in_pred_grid; level::Int64=-1, parameter::String, timestep, animation=false, save_file_name::String = "None")
      """
      Returns a stacked plot showing groundtruth and prediction componants.
      2D slice through <height> plane, shows latitudinal and longitudinal values.
      """
    ...
    # Generate plot mesh. Manually set resolution
    nθ = 73
    nλ = 144
    # 1D array with nλ or nθ entries. θc range [-π/2, π/2], λc [0, 2pi]
    θc = collect(range(-π/2, stop=π/2, length=nθ))
    λc = collect(range(0, stop=2π, length=nλ))
    λc_deg, θc_deg = λc*180/pi, θc*180/pi
    ...
    # Get global maxima and minima for colorbars over all timesteps
    cmax = max(maximum(in_true_grid), maximum(in_pred_grid))
    # cmax = maximum(in_true_grid)
    cmin = min(minimum(in_true_grid), minimum(in_pred_grid))
    # cmin = minimum(in_true_grid)
    error_max = maximum(abs.(in_true_grid -  in_pred_grid))
    error_min = 0
    #perc_error_max = 100
    #perc_error_max = maximum((abs.(in_true_grid -  in_pred_grid)./ abs.(in_true_grid)).*100) # If you want to dynamically set upper bound
    ...
    y_pred_grid = in_pred_grid[:,:,level,timestep]
    # Generate Title & Subtitle
    timestamp = timestep  # Assumes 1 hr time interval
    title = fig.suptitle(parameter*"\n$timestamp hours")
    ...
    function animate(iter)
        ...
        offset = 1  # Number of timesteps to skip ahead with each frame
        y_true_grid = in_true_grid[:,:,level,offset*iter+1]
        y_pred_grid = in_pred_grid[:,:,level,offset*iter+1]
        # Update Title timestamp
        timestamp = offset*iter # Assumes 1 hr timesteps in data
        title.set_text(parameter*"\n$timestamp hours")
    ...
    if animation
        #If animating, do so and save
        anim = PyPlot.matplotlib.animation.FuncAnimation(fig,animate,frames=90,interval=1)
        anim.save(save_file_name*".gif")
    elseif save_file_name != "None"
        # Otherwise, save if desired
        fig.savefig(save_file_name*".png")
  ...
  function CFSR_plot_error(u_error, v_error, P_error, T_error; E_max=0.4, dt=1, save_file_name)
    n = size(u_error,1)
    #x = range(1, dt*n, length=n)  # scale by largest Lyapunov exponent

    plt = Plots.plot(title="CFSR Prediction Error", legend=:topleft)
    plot!(plt, u_error, label="Eastward Velocity")
    plot!(plt, v_error, label="Northward Velocity")
    plot!(plt, P_error, label="Surface Pressure")
    plot!(plt, T_error, label="Temperature")
    plot!(plt, repeat([E_max],n), label="Max error target", linestyle=:dot)
    ylabel!(plt, "Normalized Error")
    xlabel!(plt, "Time Steps (1 step = 1 hour)")
